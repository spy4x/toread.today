...
export const restoreBackupFn = async (message: Message) => {
  try {
    if (!message || !message.attributes || !message.attributes['date']) {
      console.error(
        `Please provide a message with attribute "date" that has value in format "YYYY-MM-DD"`);
    }
    ...
    console.log(`Going to restore Firestore backup for project "${projectId}" for date "${date}" from bucket "${BACKUP_BUCKET_NAME}".`);
