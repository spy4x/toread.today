...
export class ItemService {
  constructor(private firestore: AngularFirestore, private userService: UserService, private logger: LoggerService) {}
  ...
  create(skeleton: ItemSkeleton): Promise<string> {
    ...
    return this.firestore
      .collection<Item>('items', ref =>
        ref
          .where('url', '==', itemToCreate.url)
          .where('createdBy', '==', this.userService.userId)
          .limit(1)
      )
  ...
  async remove(id: string): Promise<void> {
    ...
    try {
      await this.firestore.doc(this.getPathForId(id)).delete();
    } catch (error) {
      this.logger.error({
        messageForDev: 'ItemService.remove():',
        messageForUser: 'Failed to delete item.',
        error,
        params: { id }
      });
    }
  ...
  retryURLParsing(id: string): Promise<void> {
    return this.update(
      { id, urlParseError: null, urlParseStatus: 'notStarted' },
      'Failed to send request about' + ' "Retry URL parsing" for item.'
    );
  }
  ...
  setURL(id: string, url: string): Promise<void> {
    return this.update(
      { id, url, title: null, urlParseError: null, urlParseStatus: 'notStarted' },
      'Failed to set' + ' item URL.'
    );
  }
  ...
  async getTagIdOrCreate(tagIdOrTitle: string, userId: string, cache: Map<string, string>): Promise<string> {
    ...
    if (tagIdOrTitle.length === 20) {
      // length of Firestore document autogenerated ID
      let doc;
      try {
  ...
  private async update(data: Partial<Item>, errorMessageForUser?: string): Promise<void> {
    ...
    try {
      await this.firestore.doc(this.getPathForId(data.id)).update(body);
    } catch (error) {
      this.logger.error({
