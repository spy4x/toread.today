...
export class UserService {
  ...
  userId$ = this._firebaseUser$.pipe(
    map(u => (u ? u.uid : null)),
    shareReplay(1)
  );
  ...
  async signIn(provider: AuthMethod, email?: string, password?: string): Promise<void> {
    ...
    try {
      ...
      if (error.code === 'auth/account-exists-with-different-credential') {
        ...
        this._signError$.next(
          `You already have account for ${error.mail}. To link ${
            this.pendingCredential.providerId
          } to it - sign in with one of: ${methods.join(', ')}.`
        );
        this._isSignInProgress$.next(false);
        return;
      }
  ...
  async finishSignInWithEmailLink(): Promise<void> {
    try {
      ...
      if (!email) {
        ...
        if (!email) {
          this._signError$.next(
            "Email wasn't provided to finish Magic Link sign in. You can refresh page to try again"
          );
          return;
        }
      }
  ...
  activatePushNotifications(): void {
    this.authorizedUserOnly$
      .pipe(
        take(1),
        switchMap(() =>
          this.pushNotificationsService.getToken$().pipe(
            filter(v => !!v),
            take(1),
  ...
  async saveFCMToken(token: string): Promise<void> {
    ...
    await this.update({ fcmTokens }, 'Failed to save push notifications token.');
  }
  ...
  private async update(data: Partial<User>, errorMessageForUser?: string): Promise<void> {
    ...
    try {
      await this.firestore.doc(this.getPathForId(this.user.id)).update(body);
    } catch (error) {
      this.logger.error({
