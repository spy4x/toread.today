...
import { defaultRoadmapBrick, RoadmapBrick, RoadmapBrickType } from '../../interfaces';
import {
...
export class RoadmapComponent implements OnDestroy {
  ...
  bricksNew$: Observable<RoadmapBrick[]> = this.firestore
    .collection<RoadmapBrick>(collectionPath, ref =>
      ref
        .where('status', '==', 'new')
        .orderBy('type')
        .orderBy('score', 'desc')
    ...
    .pipe(
      ...
      catchError(error => {
        this.logger.error({
          messageForDev: 'features$ error',
          messageForUser: 'Failed to fetch new roadmap bricks.',
          error
        });
        return of([]);
      }),
  ...
  bricksInProgress$: Observable<RoadmapBrick[]> = this.firestore
    .collection<RoadmapBrick>(collectionPath, ref =>
      ref.where('status', '==', 'inProgress').orderBy('startWorkingAt', 'asc')
    )
    .valueChanges({ idField: 'id' })
    .pipe(
      takeUntil(this.userService.signedOut$),
      ...
      catchError(error => {
        this.logger.error({
          messageForDev: 'suggestions$ error',
          messageForUser: 'Failed to fetch roadmap bricks in progress.',
          error
        });
        return of([]);
      }),
  ...
  bricksDone$: Observable<RoadmapBrick[]> = this.firestore
    .collection<RoadmapBrick>(collectionPath, ref => ref.where('status', '==', 'done').orderBy('releasedAt', 'desc'))
    .valueChanges({ idField: 'id' })
    .pipe(
      takeUntil(this.userService.signedOut$),
      takeUntil(this.componentDestroy$),
      catchError(error => {
        this.logger.error({
          messageForDev: 'bugs$ error',
          messageForUser: 'Failed to fetch done roadmap bricks',
          error
        });
        return of([]);
      }),
  ...
  async vote({ brick, rate }: { brick: RoadmapBrick; rate: -1 | 1 }): Promise<void> {
    ...
    try {
      await this.firestore.doc(`${collectionPath}/${brick.id}`).update(data);
    } catch (error) {
      this.logger.error({
  ...
  async remove(id: string): Promise<void> {
    ...
    try {
      await this.firestore.doc(`${collectionPath}/${id}`).delete();
    } catch (error) {
      this.logger.error({
  ...
  async add(title: string, type: RoadmapBrickType): Promise<void> {
    ...
    try {
      await this.firestore.collection(collectionPath).add(newBrick);
    } catch (error) {
      this.logger.error({
        messageForDev: 'add() error:',
        messageForUser: 'Failed to add roadmap brick.',
        error,
        params: { newBrick }
      });
    }
  ...
  async changeTitle(event: RoadmapBrickChangeTitleEvent): Promise<void> {
    ...
    try {
      await this.firestore.doc(`${collectionPath}/${event.brick.id}`).update(data);
    } catch (error) {
      this.logger.error({
