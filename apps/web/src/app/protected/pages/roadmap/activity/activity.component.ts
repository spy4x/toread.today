...
export class RoadmapActivityComponent implements OnInit, OnDestroy {
    ...
    switchMap((userId: string) =>
      this.firestore
        ...
        .pipe(
          ...
          catchError(error => {
            this.logger.error({
              messageForDev: 'allNotifications$ error',
              messageForUser: 'Failed to fetch activity',
              error
            });
            return of([]);
          })
  ...
  constructor(private userService: UserService, private firestore: AngularFirestore, private logger: LoggerService) {}
