...
export class TagsComponent implements OnDestroy{
  ...
  async create(tag: Tag) {
    ...
    try {
      ...
      this.logger.error({messageForDev: 'createTag() error:', messageForUser: 'Failed to save tag.', error, params:{ ...tag }});
      this.error = error.message;
    }
  }
  ...
  async update(event: TagUpdateEvent) {
    try {
      ...
      this.logger.error({messageForDev: 'updateTag() error:', messageForUser: 'Failed to update tag.', error, params:{ ...event }});
      this.error = error.message;
    }
  }
  ...
  async delete(tagId: string) {
    try {
      ...
      this.logger.error({messageForDev: 'deleteTag() error:', messageForUser:'Failed to delete tag.', error, params: { tagId }});
      this.error = error.message;
    }
  }
